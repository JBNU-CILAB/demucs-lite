"""
Qualcomm AI Hub를 사용한 Demucs ONNX 모델 테스트 스크립트 (INT8 배치 버전)

FP32 모델을 컴파일 시 calibration 데이터로 PTQ(Post-Training Quantization)하여 
INT8로 양자화합니다.
"""
...
def create_calibration_data(audio: np.ndarray, num_samples: int = 50) -> dict:
    """
    오디오에서 calibration 데이터 생성
    """
    total_samples = audio.shape[1]
    ...
    # 균등하게 분포된 위치에서 샘플 추출
    step = max(1, (total_samples - CHUNK_SIZE) // num_samples)
    
    count = 0
    for start in range(0, total_samples - CHUNK_SIZE, step):
        if count >= num_samples:
            break
        
        chunk = audio[:, start:start + CHUNK_SIZE]
        
        # x: 주파수 도메인 STFT (첫 번째 입력)
        x_tensor = compute_stft_for_chunk(chunk).astype(np.float32)
        x_samples.append(x_tensor)
        
        # input: 시간 도메인 waveform (두 번째 입력)
        input_tensor = chunk.reshape(1, 2, CHUNK_SIZE).astype(np.float32)
        input_samples.append(input_tensor)
        
        count += 1
    
    print(f"  Created {len(input_samples)} calibration samples")
...
def compile_model_for_chunk(device: hub.Device, calibration_data: dict):
    print("\n" + "="*60)
    print("STEP 1: Compiling model with INT8 PTQ")
    print("="*60)
    
    print(f"  Model: {MODEL_PATH}")
    print(f"  Using calibration data for INT8 quantization...")
    
    # FP32/FP16 모델을 calibration 데이터와 함께 컴파일하면
    # 자동으로 INT8 PTQ가 적용됨
    compile_job = hub.submit_compile_job(
        model=MODEL_PATH,
        device=device,
        options="--target_runtime qnn_dlc --quantize_full_type int8",
        # x가 먼저 와야 함 (모델의 입력 순서에 맞춤)
        input_specs=dict(
            x=((1, 4, FREQ_BINS, FREQ_CHUNK_LEN), "float32"),
            input=((1, 2, CHUNK_SIZE), "float32")
        ),
    ...
    print(f"✅ Compilation with INT8 PTQ successful!")
...
def submit_inference_batch(target_model, device, chunks_batch, batch_start_idx):
    ...
    for i, (start_idx, chunk_audio) in enumerate(chunks_batch):
        # x가 먼저 와야 함
        x_tensor = compute_stft_for_chunk(chunk_audio).astype(np.float32)
        input_tensor = chunk_audio.reshape(1, 2, CHUNK_SIZE).astype(np.float32)
        
        inference_job = hub.submit_inference_job(
            model=target_model,
            device=device,
            inputs=dict(x=[x_tensor], input=[input_tensor]),  # x first
        )
...
def run_batch_inference(target_model, device, chunks, original_length, batch_size=50):
    print("\n" + "="*60)
    print(f"STEP 2: Batch inference for {len(chunks)} chunks (INT8)")
...
def save_stems_as_raw(stems: np.ndarray, output_dir: str = "output"):
    ...
    output_path = os.path.join(output_dir, "add_67_int8.raw")
...
def main():
    parser = argparse.ArgumentParser(description="Batch Demucs inference on Qualcomm AI Hub (INT8 PTQ)")
    parser.add_argument("audio_file", nargs="?", default="../day6-happy.mp3")
    parser.add_argument("--batch-size", type=int, default=BATCH_SIZE,
                        help=f"Jobs per batch (default: {BATCH_SIZE})")
    parser.add_argument("--calibration-samples", type=int, default=NUM_CALIBRATION_SAMPLES,
    ...
    print("="*60)
    print("Demucs - INT8 PTQ Batch Inference on Qualcomm AI Hub")
    print("="*60)
    print(f"Model: {MODEL_PATH}")
    print(f"Device: {DEVICE_NAME}")
    print(f"Chunk size: {CHUNK_SIZE/SAMPLE_RATE:.2f}s")
    print(f"Batch size: {BATCH_SIZE}")
    print("Quantization: INT8 (Post-Training Quantization)")
    
    device = hub.Device(DEVICE_NAME)
    
    # 1. 오디오 로드 (calibration과 추론 모두에 사용)
    print("\n" + "="*60)
    print("Loading audio")
    print("="*60)
    ...
    print("="*60)
    calibration_data = create_calibration_data(audio, args.calibration_samples)
    
    # 3. 컴파일 with INT8 PTQ
    compile_job, target_model = compile_model_for_chunk(device, calibration_data)
    if target_model is None:
        sys.exit(1)
    
    # 4. 청킹
    chunks = chunk_audio(audio, CHUNK_SIZE, HOP_SIZE)
    print(f"\n  Total chunks for inference: {len(chunks)}")
    ...
    print("  python postprocess_demucs.py output stems_int8 --raw-file add_67_int8.raw")
